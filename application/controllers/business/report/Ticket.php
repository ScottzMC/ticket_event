<?php 
    ...
    class Ticket extends CI_Controller{
        ...
        public function index(){
          ...
          if($role == "Business"){
            $data['ticket'] = $this->Business_model->display_all_ticket_by_company($session_id);
    
            $this->load->view('business/report/ticket/view', $data);
          }else{
            redirect('home');
          } 
        }
        ...
        public function category($id){
          ...
          if($role == "Business"){
            $data['id'] = $id; 
            $data['ticket'] = $this->Business_model->display_ticket_by_id($id);
            $data['report'] = $this->Business_model->display_ticket_report_by_id($id);
    
            $this->load->view('business/report/ticket/category', $data);
          }else{
            redirect('home');
          } 
        }
        ...
        public function filter($id){
          ...
          if($role == "Business"){
            $data['id'] = $id; 
            $data['ticket'] = $this->Business_model->display_ticket_by_id($id);
            $data['filter'] = $this->Business_model->display_ticket_filter_report($id, $start_date, $end_date);
    
            $this->load->view('business/report/ticket/filter', $data);
          }else{
            redirect('home');
          }
        }
