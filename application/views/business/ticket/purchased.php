...
<head>
	...
	<title>View All Purchased Tickets || Business</title>
	<!-- Favicon -->
            ...
            <div class="container-fluid">
				...
				<script>
                  ...
                  function cancel_ticket(id){
                    ...
                    $.post('<?php echo base_url('business/ticket/cancel_ticket'); ?>', {"id": id}, function(data){
                      location.reload();
                      $('#ctc').html(data)
                      });
                    }
									...
									<div class="table-wrap">
										<div class="table-responsive">
											<table id="datable_1" class="table display product-overview mb-30">
												...
												<tbody>
                                                    <?php if(!empty($purchased)){ foreach($purchased as $pur){ ?>
													<tr>
													    <td><input type="checkbox" class="checkbox" value="<?php echo $pur->id; ?>" /></td>
														<td>#<?php echo $pur->code; ?></td>
														<td><?php echo $pur->charge_id; ?></td>
                            							<td><?php echo $pur->title; ?></td>
                            							<td><?php echo $pur->customer_email; ?></td>
                            							<td><?php echo str_replace('-', ' ', $pur->events); ?></td>
                            							<td>&pound;<?php echo $pur->price; ?></td>
                            							<td>&dollar;<?php echo $pur->usd; ?></td>
                            							<td>&ush;<?php echo $pur->shilling; ?></td>
                            							<td>SLL<?php echo $pur->leone; ?></td>
                            							<td><?php echo $pur->quantity; ?></td>
                            							<td><?php echo $pur->status; ?></td>
                                                        <!--<td>
                                                            <form action="< ?php echo base_url('business/ticket/make_refund'); ?>" method="POST">
                                                                <input type="hidden" name="charge_id" value="<?php echo $pur->charge_id; ?>">
                                                        ...
                                                        <td>
													     <button type="button" onclick="cancel_ticket(<?php echo $pur->id; ?>)" class="btn btn-danger btn-icon-anim btn-square" title="Cancelled">
                                                            <i class="icon-trash"></i>
                                                          </button>
                                                        </td>
                            							<td><?php echo date("j M Y", strtotime($pur->created_date)); ?></td>
													</tr>
                                                <?php } }else{ echo ''; } ?>
    ...
    $(document).ready(function(){
     ...
     $('#cancelled_all').click(function(){
      ...
      if(checkbox.length > 0){
       ...
       $.ajax({
        url:"<?php echo base_url('business/ticket/cancelled_all_ticket'); ?>",
        method:"POST",
